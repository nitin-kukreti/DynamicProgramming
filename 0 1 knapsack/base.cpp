...
// problem is 0/1 knapsack problem
// we can pick any item only once
*/
...
//  when we not take the current item
 ...
 // if the weight of the current item is less than the capacity then we can take it
  ...
  if(items[item-1].weight<=capacity){
    //  pick and goes to the next item
    taken=items[item-1].value+recursive_kapsack(item-1,capacity-items[item-1].weight,items);
  }   
...
// when we not take the current item
 ...
 // if the weight of the current item is less than the capacity then we can take it
  ...
  if(items[item-1].weight<=capacity){
       //  pick and goes to the next item
    taken=items[item-1].value+recursive_kapsackMemo(item-1,capacity-items[item-1].weight,items,memo);
  }   
    ...
    for(int item=1;item<=n;item++){
        for(int cap=1;cap<=capacity;cap++){
            ...
            if(items[item-1].weight<=cap){
                 //  pick and goes to the next item
                taken=items[item-1].value+T[item-1][cap-items[item-1].weight];
            }
